...
// Change status mic, speaker, status
...
ipcMain.handle("change-status-status", async (event: any, data: any) => {
  let uid = store.get('uid');
  return axiosIns.post(`/users/updateStatusStatus/${uid}`)
    .then(function (response: any) {
      store.set('status_status', response.data.result[0].is_status)
      return response.data
    })
    .catch(function (error: any) {
      return error
